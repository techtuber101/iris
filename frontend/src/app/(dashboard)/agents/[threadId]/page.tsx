...
import { addUserMessage, getMessages, startAgent, stopAgent, getAgentRuns, getProject, getThread, updateProject, Project, Message as BaseApiMessageType, listSandboxFiles, type FileInfo } from '@/lib/api';
...
import { EnhancedToolPanel } from "@/components/thread/enhanced-tool-panel";
import { StatusIndicator, useStatusIndicator } from "@/components/ui/status-indicator";
import { ShareChatModal } from "@/components/share/share-chat-modal";
...
import { ArtifactChips, type Artifact } from '@/components/thread/ArtifactChips';
...
export default function ThreadPage({ params }: { params: Promise<ThreadParams> }) {
  ...
  const { status: currentStatus, message: statusMessage, updateStatus, StatusComponent } = useStatusIndicator('idle');
  ...
  const handleSubmitMessage = useCallback(async (message: string) => {
    ...
    try {
      // Follow-up mode: Always add the user message immediately.
      const sendMessagePromise = addUserMessage(threadId, message);
      let startRunPromise: Promise<any> | null = null;
      if (!agentRunId) {
        // Start a new run only if one is not already running.
        startRunPromise = startAgent(threadId);
      }

      const results = await Promise.allSettled([
        sendMessagePromise,
        ...(startRunPromise ? [startRunPromise] as const : [])
      ] as const);
      ...
      if (startRunPromise) {
        const agentResult = (results[1] as PromiseFulfilledResult<any>).value;
  ...
  // Build artifact objects from FileInfo list
  const buildArtifacts = useCallback((files: FileInfo[], sbxId: string, currentRunId?: string | null): Artifact[] => {
    const guessMime = (name: string): { mime: string; previewable: boolean } => {
      const lower = name.toLowerCase();
      if (/(\.png|\.jpg|\.jpeg|\.gif|\.webp|\.svg|\.bmp|\.ico)$/.test(lower)) return { mime: 'image/*', previewable: true };
      if (lower.endsWith('.pdf')) return { mime: 'application/pdf', previewable: true };
      if (/(\.md|\.markdown|\.txt|\.csv|\.json|\.xml|\.html|\.css|\.js|\.ts|\.py|\.log)$/.test(lower)) return { mime: 'text/plain', previewable: true };
      return { mime: 'application/octet-stream', previewable: false };
    };
    return files
      .filter(f => !f.is_dir)
      .map(f => {
        const { mime, previewable } = guessMime(f.name);
        ...
        return {
          ...
          mime,
          previewable,
          downloadUrl: `${base}/api/sandboxes/${sbxId}/files/content?path=${encodedPath}`,
          previewUrl: `${base}/api/sandboxes/${sbxId}/files/content?path=${encodedPath}`,
  ...
  useEffect(() => {
    if (projectName) {
      ...
      if (metaDescription) {
        metaDescription.setAttribute('content', `${projectName} - Interactive agent conversation powered by Iris`);
      }
      ...
      (async () => {
        try {
          if (sandboxId) {
            ...
            setLastArtifacts(buildArtifacts(files, sandboxId, agentRunId));
          }
        } catch (e) {
        ...
        >
          <div className="mx-auto max-w-3xl">
                        ...
                        <div key={group.key} ref={groupIndex === groupedMessages.length - 1 ? latestMessageRef : null}>
                          <div className="flex items-start gap-3">
                            <div className="flex-shrink-0 w-5 h-5 mt-2 rounded-md flex items-center justify-center overflow-hidden ml-auto mr-2">
                              <Image src="/iris-symbol.png" alt="Iris" width={14} height={14} className="object-contain invert dark:invert-0 opacity-70" />
                            </div>
                            <div className="flex-1">
                                  ...
                                  {(() => {
                                    ...
                                    group.messages.forEach((message, msgIndex) => {
                                      if (message.type === 'assistant') {
                                        ...
                                        // Extract lightweight attachment markers embedded in content
                                        const attachmentMatches = ((): string[] => {
                                          try {
                                            const raw = parsedContent.content || '';
                                            const matches = raw.match(/\[Uploaded File: (.*?)\]/g) || [];
                                            return matches.map(m => {
                                              const inner = m.match(/\[Uploaded File: (.*?)\]/);
                                          ...
                                          <div key={msgKey} className={msgIndex > 0 ? "mt-2" : ""}>
                                            ...
                                            {attachmentMatches.length > 0 && (
                                              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                {attachmentMatches.map((attachment, idx) => {
                                                  const filename = attachment.split('/').pop() || 'file';
                                                  return (
                                                    <button
                                                      key={`${msgKey}-att-${idx}`}
                                                      onClick={() => handleOpenFileViewer(attachment)}
                                                      className="group flex items-center gap-3 p-3 rounded-md bg-muted/10 hover:bg-muted/20 transition-colors border border-border/50 text-left"
                                                      title={attachment}
                                                    >
                                        ...
                                        if (typeof body === 'string' && body.includes('[Uploaded File:')) {
                                          const attachmentMatches = (body.match(/\[Uploaded File: (.*?)\]/g) || []).map(m => {
                                            const inner = m.match(/\[Uploaded File: (.*?)\]/);
                                            return inner ? inner[1] : '';
                                          }).filter(Boolean);
                                          if (attachmentMatches.length > 0) {
                                            const msgKey = message.message_id || `toolmsg-${msgIndex}`;
                                            elements.push(
                                              <div key={msgKey} className="mt-2">
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                  {attachmentMatches.map((attachment, idx) => (
                                                    ...
                                                    >
                                                      <File className="h-4 w-4 text-muted-foreground" />
                                                      <span className="text-sm font-medium truncate">{attachment.split('/').pop() || 'file'}</span>
                                                    </button>
                                                  ))}
                                                </div>
                                              </div>
                    ...
                    <div ref={latestMessageRef}>
                      <div className="flex items-start gap-3">
                        <div className="flex-shrink-0 w-5 h-5 rounded-md flex items-center justify-center overflow-hidden bg-primary/10">
                          <Image src="/iris-symbol.png" alt="Iris" width={14} height={14} className="object-contain"/>
                        </div>
                        <div className="flex-1 space-y-2">
            ...
            {lastArtifacts.length > 0 && (
              <div className="mt-4">
                <ArtifactChips artifacts={lastArtifacts} onOpen={handleOpenFileViewer} />
              </div>
            )}
      ...
      <div className={cn(
        ...
        <div className={cn(
          ...
          <ChatInput
            ...
            // Update placeholder to reference Iris instead of Suna.
