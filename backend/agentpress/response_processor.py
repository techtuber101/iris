...
class ResponseProcessor:
    ...
    def __init__(self, tool_registry: ToolRegistry, add_message_callback: Callable):
        ...
        # Initialize new XML parser and tool executor
        self.xml_parser = XMLToolParser()
        self.tool_executor = ToolExecutor(tool_registry)
        # Optional hook: awaited exactly once before the first tool execution
        self.pre_tool_hook: Optional[Callable[[], Any]] = None
        self._pre_tool_hook_used = False
    ...
    async def process_streaming_response(
        ...
        try:
                            ...
                            for xml_chunk in xml_chunks:
                                ...
                                if result:
                                    ...
                                    if config.execute_tools and config.execute_on_stream:
                                        # Ensure any required environment is ready (lazy sandbox)
                                        if self.pre_tool_hook and not self._pre_tool_hook_used:
                                            try:
                                                hook = self.pre_tool_hook
                                                self._pre_tool_hook_used = True
                                                ...
                                                logger.error(f"pre_tool_hook failed: {e}")
            ...
            if pending_tool_executions:
                ...
                # Heartbeat loop: wait in slices and emit a heartbeat event
                while True:
                    if not pending_tasks:
                        break
                    done, pending = await asyncio.wait(pending_tasks, timeout=2.0, return_when=asyncio.FIRST_COMPLETED)
                    if not done:
                        # Emit heartbeat status message
                        hb_content = {"status_type": "heartbeat", "message": "Working..."}
                        hb_msg_obj = await self.add_message(
                            thread_id=thread_id, type="status", content=hb_content,
                            is_llm_message=False, metadata={"thread_run_id": thread_run_id}
                        )
                        if hb_msg_obj:
                            yield hb_msg_obj
                    ...
                    finished_map = {t: i for i, t in enumerate([e["task"] for e in pending_tool_executions])}
                    for t in done:
                        try:
                            idx = finished_map.get(t, -1)
                            if idx == -1:
                                continue
                            execution = pending_tool_executions[idx]
                            ...
                            try:
                                ...
                                # If we didn't already yield a status for this tool, do it now
                                ...
                                # Capture exceptions and yield error status
                                context.error = e
                                tool_results_buffer.append((execution["tool_call"], ToolResult(success=False, output=None, error=str(e)), tool_idx, context))
                                error_msg_obj = await self._yield_and_save_tool_error(context, thread_id, thread_run_id)
                                if error_msg_obj:
                                    yield error_msg_obj
                                yielded_tool_indices.add(tool_idx)
                        except Exception:
                            # Defensive: continue processing other tasks
                            pass
                    pending_tasks = list(pending)
            ...
            if accumulated_content:
                ...
                if last_assistant_message_object:
                    ...
                    try:
                        raw_meta = last_assistant_message_object.get('metadata', '{}')
                        if isinstance(raw_meta, dict):
                            yield_metadata = raw_meta
                        else:
                            yield_metadata = json.loads(raw_meta or '{}')
                    except Exception:
                        yield_metadata = {}
    ...
    def _extract_xml_chunks(self, content: str) -> List[str]:
        """Extract complete XML chunks using the new robust XML parser."""
        try:
            # Use the new XML parser
            return self.xml_parser.extract_xml_chunks(content)
        except Exception as e:
    ...
    async def _execute_tool(self, tool_call: Dict[str, Any]) -> ToolResult:
        """Execute a single tool call with timeout and basic retry for transient errors."""
        try:
            ...
            # Determine timeout from args (seconds) with sane default
            timeout_s = 120
            try:
                if isinstance(arguments, dict) and 'timeout' in arguments and isinstance(arguments['timeout'], (int, float)):
                    timeout_s = max(1, int(arguments['timeout']))
            except Exception:
                pass
            ...
            # Try once, with one retry for transient sandbox errors
            attempts = 0
            ...
            while attempts < 2:
                ...
                try:
                    ...
                    logger.info(f"Tool execution complete: {function_name} (attempt {attempts})")
                    return result
                except asyncio.TimeoutError as te:
                    logger.error(f"Tool timeout: {function_name} after {timeout_s}s (attempt {attempts})")
                    last_err = te
                    break  # do not retry on timeout
                except Exception as e:
                    # Retry only once for likely transient sandbox errors
                    err_text = str(e).lower()
                    transient = any(k in err_text for k in ["temporary", "econn", "connection reset", "try again", "timeout"])
                    logger.error(f"Tool error: {function_name} (attempt {attempts}): {e}")
                    last_err = e
            ...
            return ToolResult(success=False, output=None, error=str(last_err) if last_err else "unknown error")
        except Exception as e:
            logger.error(f"Error executing tool {tool_call.get('function_name', '?')}: {str(e)}", exc_info=True)
            return ToolResult(success=False, output=None, error=f"Error executing tool: {str(e)}")

    async def process_xml_tools_with_canonical_events(
        self, 
        xml_content: str,
        **execution_context
    ) -> AsyncGenerator[Dict[str, Any], None]:
        """
        Process XML content and emit canonical tool_call and tool_result events.
        
        This method uses the new XML parser and tool executor to provide
        canonical event emission as specified in the requirements.
        
        Args:
            xml_content: XML content containing tool calls
            **execution_context: Additional context for tool execution
            
        Yields:
            Canonical tool_call and tool_result events
        """
        try:
            # Parse XML content using the new parser
            tool_calls = self.xml_parser.parse_xml_tools(xml_content)
            
            if not tool_calls:
                logger.debug("No tool calls found in XML content")
                return
            
            logger.info(f"Found {len(tool_calls)} tool calls to execute")
            
            # Execute tools and emit canonical events
            async for event in self.tool_executor.execute_tool_calls(tool_calls, **execution_context):
                yield event
                
        except Exception as e:
            logger.error(f"Error processing XML tools: {e}")
            yield {
                "type": "tool_result",
                "tool": "unknown",
                "output": None,
    ...
    async def _yield_and_save_tool_started(self, context: ToolExecutionContext, thread_id: str, thread_run_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            logger.info(
                f"TOOL_CALL_START: {{'runId': '{thread_run_id}', 'tool': '{context.function_name}', 'toolIndex': {context.tool_index}, 'startedAt': '{datetime.now(timezone.utc).isoformat()}'}}"
            )
        except Exception:
            pass
    ...
    async def _yield_and_save_tool_completed(self, context: ToolExecutionContext, tool_message_id: Optional[str], thread_id: str, thread_run_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            payload = {
                'runId': thread_run_id,
                'tool': context.function_name,
                'toolIndex': context.tool_index,
                'finishedAt': datetime.now(timezone.utc).isoformat(),
                'outcome': 'ok' if context.result.success else 'error'
            }
            logger.info(f"TOOL_CALL_FINISH: {payload}")
        except Exception:
            pass
    ...
    async def _yield_and_save_tool_error(self, context: ToolExecutionContext, thread_id: str, thread_run_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"TOOL_CALL_ERROR: {payload}")
        except Exception:
            pass
        return saved_message_obj
