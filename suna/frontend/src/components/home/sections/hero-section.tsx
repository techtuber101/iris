...
export function HeroSection() {
  ...
  // Split the hero title on the first comma to apply different colors to each part
  const titleParts = typeof hero.title === 'string' ? hero.title.split(',') : [];
  const heroFirstPart = titleParts[0] || hero.title;
  // Include the comma and remainder (if any) as the second part
  const heroSecondPart = titleParts.length > 1 ? `,${titleParts.slice(1).join(',')}` : '';
        ...
        <div className="relative z-10 pt-32 max-w-3xl mx-auto h-full w-full flex flex-col gap-10 items-center justify-center">
          ...
          <div className="flex flex-col items-center justify-center gap-5 pt-16">
            <h1 className="text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-medium tracking-tighter text-balance text-center">
              <span className="text-secondary">{heroFirstPart}</span>
              <span className="text-primary">{heroSecondPart}</span>
            </h1>
            ...
            <div className="w-full relative">
              <div className="relative z-10">
                <ChatInput
                  ...
                  placeholder={hero.inputPlaceholder || 'Describe what you need help with...'}
